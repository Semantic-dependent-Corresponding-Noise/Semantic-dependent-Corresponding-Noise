...
1. Generate Base Noise Files
   Command: python yibu_batch_core_missing_noise_doubao_construct.py
...
### II. Generate Test Set Noise
   Command: python yibu_batch_core_missing_test_noise_doubao_construct.py
...
- All Error (1 img, 5 caps, all errors):
  Command: python core_missing_img5txt_noise.py
  Description: All descriptions are erroneous
...
## Important Notes
1. Execution order: Generate → Merge → Filter
2. Generating 100% error noise is the foundation for subsequent operations
3. Ensure sufficient disk space
4. All scripts should be run from the project root directory
